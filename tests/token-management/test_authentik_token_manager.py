...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../scripts/token-management'))
...
"""
    ...
    @patch.object(authentik_token_manager, 'run_command', return_value="authentik-enhanced-token-setup-abc123")
    ...
    @patch.object(authentik_token_manager, 'get_current_token', return_value="test_token")
    ...
    def test_token_validation(self):
        ...
        for invalid_token in invalid_tokens:
            with self.subTest(token=invalid_token):
                ...
                is_valid = (len(invalid_token) == 64 and 
                           all(c in '0123456789abcdef' for c in invalid_token))
