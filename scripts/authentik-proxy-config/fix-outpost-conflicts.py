...
class OutpostConflictResolver:
    ...
    def __init__(self, authentik_host: str, authentik_token: str, external_outpost_id: str):
        ...
        self.services = [
            ServiceConfig("longhorn", "longhorn.k8s.home.geoffdavis.com", 
                        "longhorn-frontend.longhorn-system.svc.cluster.local", 80),
            ...
            ServiceConfig("dashboard", "dashboard.k8s.home.geoffdavis.com", 
                        "kubernetes-dashboard-kong-proxy.kubernetes-dashboard.svc.cluster.local", 443),
    ...
    def update_proxy_provider(self, provider_pk: int, service: ServiceConfig, auth_flow_uuid: str) -> bool:
        ...
        try:
            self.logger.info(f"Updating proxy provider {service.name}-proxy (PK: {provider_pk})")
            ...
            if status_code == 200:
                ...
                self.logger.error(f"✗ Failed to update proxy provider {service.name}-proxy: status {status_code}")
                return False
                
        except AuthentikAPIError as e:
            self.logger.error(f"✗ Failed to update proxy provider {service.name}-proxy: {e}")
            return False
    ...
    def get_authorization_flow(self) -> str:
        ...
        try:
            ...
            fallback_uuid = "be0ee023-11fe-4a43-b453-bc67957cafbf"
            self.logger.warning(f"⚠ Failed to get authorization flow: {e}, using fallback: {fallback_uuid}")
            return fallback_uuid
    
    def update_external_outpost_config(self) -> bool:
        """Update external outpost configuration to clear cached internal URLs."""
        try:
            self.logger.info(f"Updating external outpost configuration: {self.external_outpost_id}")
            ...
            if status_code == 200:
                ...
                self.logger.info("✓ External URL set to: https://authentik.k8s.home.geoffdavis.com")
                return True
            else:
    ...
    def resolve_conflicts(self) -> bool:
        ...
        for service in self.services:
            ...
            if provider_name in proxy_providers:
                ...
                self.logger.info(f"✓ Found provider: {provider_name} (PK: {provider_pk})")
                ...
                if not self.update_proxy_provider(provider_pk, service, auth_flow_uuid):
                    self.logger.warning(f"⚠ Failed to update provider {provider_name}, but continuing...")
            else:
        ...
        for outpost_id, outpost_info in outposts.items():
            ...
            if outpost_id == self.external_outpost_id:
                ...
                conflicting_providers = [pk for pk in assigned_providers if pk in expected_provider_pks]
                if conflicting_providers:
                    competing_outposts.append((outpost_id, outpost_name, conflicting_providers))
                    self.logger.warning(f"  → This outpost has CONFLICTING providers: {conflicting_providers}")
        ...
        for outpost_id, outpost_name, conflicting_providers in competing_outposts:
            self.logger.info(f"Removing providers from competing outpost: {outpost_name} ({outpost_id})")
        ...
        if not self.update_external_outpost_config():
            self.logger.warning("⚠ Failed to update outpost configuration, but continuing...")
        ...
        self.logger.info("✓ Services should now work correctly with proper authentication redirects")
...
def main():
    ...
    external_outpost_id = os.environ.get('EXTERNAL_OUTPOST_ID', '3f0970c5-d6a3-43b2-9a36-d74665c6b24e')
    ...
    resolver = OutpostConflictResolver(authentik_host, authentik_token, external_outpost_id)
