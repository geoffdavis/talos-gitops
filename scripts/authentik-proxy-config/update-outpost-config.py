...
def main():
    ...
    status_code, response = make_api_request(url, method='PATCH', data=update_data, headers=headers)
    
    if status_code == 200:
        print("✓ Successfully updated outpost configuration")
        print("✓ External URL now set to: https://authentik.k8s.home.geoffdavis.com")
        print("✓ Outpost should now redirect to external URL instead of internal cluster DNS")
        return True
    else:
