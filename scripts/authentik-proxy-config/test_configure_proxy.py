...
class TestAuthentikProxyConfigurator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.configurator = AuthentikProxyConfigurator(
            self.config, 
            logger=self.logger
        )
    ...
    def test_configurator_initialization(self):
        ...
        expected_names = [
            "longhorn", "grafana", "prometheus", 
            "alertmanager", "dashboard", "hubble"
        ]
    ...
    def test_make_api_request_http_error(self, mock_urlopen):
        ...
        with self.assertRaises(AuthentikAPIError) as context:
            self.configurator._make_api_request(
                "https://auth.example.com/api/test"
            )
    ...
    def test_create_proxy_provider_success(self, mock_urlopen):
        ...
        service = ServiceConfig(
            "test", "test.example.com", "test-service", 8080
        )
        
        provider_pk = self.configurator.create_proxy_provider(
            service, "test-flow-uuid"
        )
    ...
    def test_update_outpost_providers_success(self, mock_urlopen):
        ...
        self.assertTrue(result)
        log_output = self.log_stream.getvalue()
        self.assertIn("Updated outpost test-outpost with providers: [1, 2, 3]", log_output)
    ...
    def test_configure_all_services_success(
        self, mock_update_outpost, mock_create_app, mock_create_provider,
        mock_get_apps, mock_get_providers, mock_get_flow, mock_test_auth
    ):
