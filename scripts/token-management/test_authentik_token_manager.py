...
class TestAuthentikTokenManager(unittest.TestCase):
    ...
    def test_create_long_lived_token_success(
        self, mock_wait, mock_prereq, mock_shell
    ):
    ...
    def test_list_tokens_success(self, mock_wait, mock_prereq, mock_shell):
        ...
        mock_shell.return_value = (
            True,
            f"TOKEN_LIST:{json.dumps(token_list)}"
        )
    ...
    def test_create_shell_job_manifest(self):
        ...
        manifest = self.manager._create_shell_job_manifest(
            "test-job", python_code
        )
